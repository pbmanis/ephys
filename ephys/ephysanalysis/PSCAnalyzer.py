...
class PSCAnalyzer:
    ...
    def measure_PSC(
        self, protocolName, plot=True, savetimes=False, ignore_important_flag=True
    ):
        """
        Direct the analysis
    ...
    def analyze_IO(
        ...
        self.analysis_summary[f"PSP_IO"] = [[]] * len(
            pulse_train["start"]
        )  # create space for amplitude results, per pulse
        self.analysis_summary[f"psc_stim_amplitudes"] = [[]] * len(
            pulse_train["start"]
        )  # create space for amplitude results, per pulse
        stimintvl = []
        ...
        for i in range(len(idat)):  # across each of the pulses in the train
            ...
            if (
                i == 0 and self.update_regions
            ):  # if self.update_region is set, then use cursor plot to get the regions
                rgn = self.set_region(
                    [pulse_train["start"][i], pulse_train["start"][i] + twidth],
                    baseline=bl,
                )
            ...
            self.analysis_summary[f"PSP_IO"][i] = (
                self.sign * 1e12 * np.array([idat[i][k] for k in idat[i].keys()])
            )
            self.analysis_summary[f"psc_stim_amplitudes"][i] = 1e6 * np.array(
    ...
    def analyze_PPF(
        ...
        self.reject_list = []
        for rj in self.NGlist:
            trial = int(rj[0:3])
            intvl_trace = int(rj[4:])
            self.reject_list.append((trial * len(reps)) + intvl_trace)
        ...
        for rj in self.NGlist:
            trial = int(rj[0:3])
            intvl_trace = int(rj[4:])
            self.reject_list.append((trial * len(stim_dt)) + intvl_trace)
        #     print(trial, intvl_trace, len(stim_dt), self.reject_list[-1])
        # cprint.cprint('c', f"NGlist: {str(self.NGlist):s}")
        ...
        for j in range(len(self.AR.traces)):  # for all (accepted) traces
            if j in self.reject_list:
                print('*'*80)
                print(f"trace j={j:d} is in rejection list: {str(self.reject_list):s}")
                print(f"     from: {str(self.NGlist):s}")
                print('*'*80)
                continue
            mi = self.AR.trace_index[
                j
            ]  # get index into marked/accepted traces then compute the min value minus the baseline
            t_stim1 = self._compute_interval(
                x0=pulse_train["start"][0],
            ...
            t_stim2 = self._compute_interval(
                x0=Stim_Intvl[mi] + pulse_train["start"][0],
                artifact_delay=dead_time,
                index=mi,
                stim_intvl=Stim_Intvl,
                max_width=twidth,
            ...
            i_pp1 = (
                self.Clamps.traces["Time": t_stim1[0]: t_stim1[1]][j] - bl
            )  # first pulse trace
            tb_ref = self.Clamps.time_base[
                np.where(
                    (self.Clamps.time_base >= t_stim1[0])
                    & (self.Clamps.time_base < t_stim1[1])
                )
            ]
            i_pp2 = (
                self.Clamps.traces["Time": t_stim2[0]: t_stim2[1]][j] - bl
            )  # second pulse trace
            tb_p2 = self.Clamps.time_base[
                np.where(
                    (self.Clamps.time_base >= t_stim2[0])
                    & (self.Clamps.time_base < t_stim2[1])
                )
            ]
    ...
    def analyze_VDEP(self, rmpregion=[0.0, 0.05], protocolName=None, device="Stim0"):
        """
        ...
        nwidth = 0.0025
        bl_region = [
            pulse_train["start"][0] - 0.060,
            pulse_train["start"][0] - 0.010,
        ...
        if self.update_regions:
            rgn = self.set_region(
                [
                    pulse_train["start"][0],
                    pulse_train["start"][0] + self.NMDA_delay + 0.010,
                ],
                baseline=bl,
                slope=True,
            )
        ...
        bl = self.mean_I_analysis(
            region=[
                pulse_train["start"][0] + self.T0 - 0.0005,
                pulse_train["start"][0] + self.T0,
            ],
            mode="baseline",
            reps=[0],
        )

        data1, tb = self.get_traces(
            region=slope_region,
            trlist=None,
            baseline=bl,
            intno=0,
        ...
        i_mean = self.mean_I_analysis(
            region=[p1delay, p1end], mode="min", baseline=bl, reps=reps, slope=False
        )
        ...
        i_nmda_mean = self.mean_I_analysis(
        ...
        if data1 is None or iNMDA >= data1.shape[0]:
            ...
            self.analysis_summary["AMPA_NMDA_traces"] = {
                "T": None,
                "iAMPA": None,
                "iNMDA": None,
            }
            ...
            self.analysis_summary["NMDAAMPARatio"] = (
                self.analysis_summary[f"PSP_VDEP_NMDA"][0][iNMDA]
                / self.analysis_summary[f"PSP_VDEP_AMPA"][0][iAMPA]
            )
            self.analysis_summary["AMPA_NMDA_traces"] = {
                "T": tb,
                "iAMPA": data1[iAMPA],
    ...
    def mean_I_analysis(
        ...
        self.V_cmd = (
            self.Clamps.cmd_wave[:, rgn[0]: rgn[1]].mean(axis=1).view(np.ndarray)
        )
        tb = np.arange(
            0, data1.shape[1] * self.Clamps.sample_interval, self.Clamps.sample_interval
        )
    ...
    def vcss_analysis(self, region=None):
        ...
        if len(self.vcss_vcmd) > 1 and len(self.vcss_v) > 1:
            ...
            l_sl = int(len(slope) / 2)
            maxloc = np.argmax(slope[l_sl:]) + l_sl
    ...
    def plot_vciv(self):
        ...
        if "PSP_IO" in self.analysis_summary.keys():  # io function
            for i in range(len(self.analysis_summary["stim_times"])):
                try:
                    P.axdict["C"].plot(
                        self.analysis_summary["psc_stim_amplitudes"],
            ...
            for i in range(len(self.analysis_summary["stim_times"])):
                P.axdict["C"].plot(
                    self.analysis_summary["Vcmd"] * 1e3,
                    self.sign * np.array(self.analysis_summary[f"PSP_VDEP"][i]) * 1e12,
                    marker="o",
                    linewidth=1,
                    markersize=4,
                )
            ...
            for i in range(len(self.analysis_summary["stim_times"])):
                P.axdict["C"].plot(
                    self.analysis_summary["ppf_dt"] * 1e3,
                    self.sign * np.array(self.analysis_summary[f"PPF"]),
                    linewidth=1,
                    markersize=4,
                )
