...
class MetaArray(object):
    ...
    def __init__(
        self, data=None, info=None, dtype=None, file=None, copy=False, **kwargs
    ):
            ...
            for i in range(len(info)):
                ...
                if i < self.ndim and "values" in info[i]:
                    if type(info[i]["values"]) is list:
                        ...
                        raise Exception(
                            "Axis values must be specified as list or ndarray"
                        )
                    if (
                        info[i]["values"].ndim != 1
                        or info[i]["values"].shape[0] != self.shape[i]
                    ):
                        raise Exception(
    ...
    def axisValues(self, axis):
        ...
        if "values" in self._info[ax]:
            ...
            raise Exception(
                "Array axis %s (%d) has no associated values." % (str(axis), ax)
            )
    ...
    def _interpretIndex(self, ind, pos, numOk):
        ...
        if type(ind) is int:
            if not numOk:
                raise Exception(
                    "string and integer indexes may not follow named indexes"
                )
        ...
        with open(filename, "rb") as fd:
            ...
            if magic == b"\x89HDF\r\n\x1a\n":
                ...
                if not hasattr(MetaArray, rFuncName):
                    raise Exception(
                        "This MetaArray library does not support array version '%s'"
                        % ver
                    )
    ...
    def _readData1(self, fd, meta, mmap=False, **kwds):
        ...
        for ax in meta["info"]:
            if "values_len" in ax:
                ax["values"] = np.fromstring(
                    fd.read(ax["values_len"]), dtype=ax["values_type"]
                )
    ...
    def _readData2(self, fd, meta, mmap=False, subset=None, **kwds):
        ...
        if dynAxis is None:
            ...
            if meta["type"] == "object":
                ...
                if mmap:
                    subarr = np.memmap(
                        fd, dtype=meta["type"], mode="r", shape=meta["shape"]
                    )
                else:
                    subarr = np.fromstring(fd.read(), dtype=meta["type"])
        ...
        ## One axis is dynamic, read in a frame at a time
            ...
            while True:
                ...
                if data.size != frameSize * inf["numFrames"]:
                    ...
                    raise Exception(
                        "Wrong frame size in MetaArray file! (frame %d)" % n
                    )
    ...
    def _readHDF5(self, fileName, readAllData=None, writable=False, **kargs):
        ...
        if readAllData is True and writable is True:
            raise Exception(
                "Incompatible arguments: readAllData=True and writable=True"
            )
        ...
        if (
            writable or not readAllData
        ):  ## read all data, convert to ndarray, close file
            self._data = f["data"]
            self._openFile = f
        else:
            self._data = f["data"][:]
    ...
    def readHDF5Meta(root, mmap=False):
        ...
        for k in root.attrs:
            ...
            if isinstance(
                val, basestring
            ):  ## strings need to be re-evaluated to their original types
                try:
                    if val[-1] == "L":
                        val = val[:-1]
                    val = eval(val)
