...
class ScannerInfo(object):
    ...
    def __init__(self, AR, offset: Union[list, np.ndarray] = [0.0, 0.0]):
...
class MosaicReader(object):
    ...
    def addItem(self, item=None, type=None, **kwds):
        ...
        *type* which is a string specifying the type of item to create and add.
        ...
        if isinstance(item, Qt.QtWidgets.QGraphicsItem):
            print("was qt")
...
class MapTraces(object):
    def __init__(self):
        ...
        self.SI_minmax = [0.0, 1.0]
        self.SI_original_minmax = [0.0, 1.0]
        self.cellpos = None
        ...
        self.offset = [0.0, 0.0]  # offset between pos and image
    ...
    def setProtocol(self, cell, image=None, videos=None, mosaic=None):
        ...
        if image is not None:
            ...
            if "userTransform" not in list(self.AR._index[imagefile.name].keys()):
                ...
                self.refpos = self.AR._index[imagefile.name]["userTransform"][
                    "pos"
                ]  # use repositioned image location
    ...
    def filter_data(self, tb, data, LPF=3000.0):
        self.HPF_flag = False
        self.LPF = LPF
        self.maxtime = 0.599  # sec
        filtfunc = scipy.signal.filtfilt
        samplefreq = 1.0 / np.mean(np.diff(tb))
        rate = 1.0 / samplefreq
    ...
    def plot_maps(self, protocols, ax=None, traces=None, linethickness=1.0, tbar=False, axb=None):
        ...
        if ax is None:
            ...
            self.figure.set_size_inches(14.0, 8.0)
            if traces is None:
    ...
    def adjust(self, extent, a):
        ...
        minx = self.SI.centroid_sh.x - a / 2.0
        maxx = self.SI.centroid_sh.x + a / 2.0
        miny = self.SI.centroid_sh.y - a / 2.0
        maxy = self.SI.centroid_sh.y + a / 2.0
    ...
    def show_traces(self, ax, pcolor="r", linethickness=0.5, name=None):
        ...
        if self.averageScannerImages:
            self.max_camera = self.AR.getAverageScannerImages(
                dataname="Camera/frames.ma",
                mode="max",
                subtractFlag=True,
                firstonly=False,
                limit=None,
            ...
            self.extent0 = [
                np.min(self.SI.boxw[0]),
                np.max(self.SI.boxw[0]),
                np.min(self.SI.boxw[1]),
                np.max(self.SI.boxw[1]),
            ]
            self.extent = self.adjust(self.extent0, mapwidth)
            self.SI_ax = ax.imshow(
                sm,
                aspect="equal",
                cmap="Blues",
                alpha=sc_alpha,
        ...
        if self.image is not None:
            ...
            # mpl.show()
            self.extent0 = [
                np.min(self.ImgInfo.boxw[0]),
                np.max(self.ImgInfo.boxw[0]),
                np.min(self.ImgInfo.boxw[1]),
                np.max(self.ImgInfo.boxw[1]),
            ]
            self.extent = self.adjust(self.extent0, mapwidth)
            self.imageax = ax.imshow(
                np.asarray(self.image_data, dtype=float),
                aspect="equal",
                cmap=cmap,
                alpha=image_alpha,
                vmin=0,
        ...
        if self.window:
            if self.xlim == (0.0, 0.0) and self.ylim == (0.0, 0.0):
                xylims = self.extent
        ...
        if box is not None:
            PH.calbar(
                ax,
                calbar=[box[0] + 0.00012, box[3] - 0.00005, 0.0001, 0],
                scale=[1e6, 1e6],
                axesoff=True,
                orient="left",
    ...
    def plot_calbar(self, ax, x0, y0):
        ...
        self.calbarobj = ax.plot(
            self.xscale * 3.5e-5 * np.array([0.0, 0.0, self.calbar[0]]) + x0 - xcal,
            (self.yscale * self.vscale * (np.array([self.calbar[1], 0.0, 0.0]) - zero))
            + self.off * self.vscale
            + y0
            - ycal,
            "k-",
            linewidth=1,
        )
        self.calbartext = ax.text(
            x0 - xcal,
            y0 - ycal,
            f"{int(self.calbar[0]*1e3):d} ms\n{int(self.calbar[1]*1e12):d} pA",
    ...
    def _plot_one(
        ...
        zero = 0.0
        vdat = FILT.SignalFilter_LPFBessel(
            self.datasets[name][p, :], 2000.0, samplefreq=self.AR.sample_rate[0], NPole=8
        )
        if self.basezero:
        ...
        ax.plot(
            ts * self.tb + xoff,
            (y_scale * (vdat[self.im0 : self.im1] - zero)) + yoff,
            color=pcolor,
            linewidth=ythick,
        )
        if self.ticks is not None:
            for t in self.ticks:
                ax.plot(
                    ts * np.array([t, t]) + xoff,
                    y_scale * np.array([-20e-12, 20e-12]) + yoff,
                    color="k",
                    linewidth=0.8,
                )
    ...
    def handle_event(self, index):
        ...
        self.ax.text(
            self.SI.scanner_positions[index][0],
            self.SI.scanner_positions[index][1],
            f"{trn:d}",
            fontsize=9,
            horizontalalignment="center",
        )
        self.ax2.text(0.0, ystep, f"{trn:d}", fontsize=9, horizontalalignment="right")
        self.nspots += 1
    ...
    def _getAngle(self, pt1: Point2D, pt2: Point2D):
        """
    ...
    # center = Point2d(self.cellpos[0], self.cellpos[1])  # center (flip y axis)
    ...
    #     e = sympy.Point([event.xdata, event.ydata])  # point in direction for top of T bar
    #     t = sympy.Point([self.tbar_coords.xy[0][2], self.tbar_coords.xy[1][2]])
    ...
    def compute_sector_distance_map(self):
        ...
        if not self.scholl_plot:
            ...
            for i, rad in enumerate(r):
                self.C_rings[i] = SG.Polygon(
                    [
                        (rad * np.sin(theta) + t[0], rad * np.cos(theta) + t[1])
        ...
        x = (
            self.SI.scanner_positions.T
        )  # self.eventdata['positions'].T  # just for the way we handle the values here.
        nsectors = 4
        rot_angle = np.pi / nsectors
        point_angles = (
            np.arctan2(x[1, :] - t[1], x[0, :] - t[0]) - self.tbar_angle
        )  # get angles relative to cell position, then rotate by tbar angle
        try:
            point_angles = np.where(
                point_angles < -rot_angle, point_angles + 2 * np.pi, point_angles
            )
        except:
        ...
        for i in range(ptlocs.shape[0]):
            u = np.where(ptlocs[i, :] == 1)[
                0
            ]  # get the index of the first ring where the point is found
        ...
        ]  # ring index by sectors,but value stored is total zscore....
        ...
        for i in range(nrings):
            ...
            for j in range(nsectors):
                siri = list(
                    set(self.angle_group[j]).intersection(ri)
                )  # get those points in this sector
                if len(siri) > 0:
                    # secdistmap[i, j] = np.mean(self.eventdata['ZScore'][0][siri])
                    secdistmap[i, j] = np.sum(self.eventdata[measure][0][siri] > thresh)

        center_of_mass = [0.0, 0.0]  # relative to cell body.
        for i in range(0, 2):
        ...
        for i in range(nsectors):
            ...
            for j in range(2):
                sector_center_of_mass[i][j] = np.sum(
                    (self.eventdata[measure][0][self.angle_group[i]] > thresh)
                    * (x[j, self.angle_group[i]])
                )
                try:
                    sector_center_of_mass[i][j] /= np.sum(
                        self.eventdata[measure][0][self.angle_group[i]] > thresh
        ...
        self.radlines = [
            (
                (t[0], rad * np.sin(theta - self.tbar_angle) + t[0]),
                (t[1], rad * np.cos(theta - self.tbar_angle) + t[1]),
            )
            for theta in sector_angles
        ]
        cl = ["r", "r", "b", "b", "y", "y"]
        ...
        if self.ref_angles is None:
            ...
            self.polydata = SG.Polygon([L[0][0], L[0][1], L[1][1], L[0][0]])
    ...
    def plot_scholl(self):
        for i in range(len(self.C_rings)):
            self._plot_coords(
                self.ax, linear_ring=self.C_rings[i].exterior, c="#888888", linewidth=0.66
            )
        for i in range(len(self.radlines)):
            if self.ref_angles[i] is None:
                self.ref_angles[i] = self.ax.plot(
                    self.radlines[i][0], self.radlines[i][1], c="gray", linewidth=0.5
                )
            else:
                self.ref_angles[i][0].set_xdata(self.radlines[i][0])
                self.ref_angles[i][0].set_ydata(self.radlines[i][1])
        self._plot_coords(
            ax=self.ax, linear_ring=self.polydata.exterior, c="#000000", linewidth=2, alpha=1.0
        )
        ...
        for i in range(self.nsectors):
            ...
            self.ax.plot(
                self.sector_center_of_mass[i, 0],
                self.sector_center_of_mass[i, 1],
                c=self.edgecolor[i],
                marker=self.sectorsymbol[i],
                markersize=8 * self.mass[i] / maxmass,
                alpha=0.75,
            )
...
def main():
    ...
    if args.celltype == "lsps":  # special
        cell = Path(
            "/Users/pbmanis/Desktop/Data/Glutamate_LSPS_DCN/2019.08.05_000/slice_002/cell_000/LSPS_dendrite_VC_testmap_MAX_001"
        )  # pyr
        image = "../image_001.tif"
        MT.setPars(
            {
        ...
        cell1 = Path(
            "/Users/pbmanis/Desktop/Data/Glutamate_LSPS_DCN/2019.08.08_000/slice_001/cell_000/LSPS_dendrite_CC_testmap_MAX_000"
        )
        cell2 = Path(
            "/Users/pbmanis/Desktop/Data/Glutamate_LSPS_DCN/2019.08.08_000/slice_001/cell_000/LSPS_dendrite_CC_testmap_MAX_001"
        )
        cell3 = Path(
            "/Users/pbmanis/Desktop/Data/Glutamate_LSPS_DCN/2019.08.08_000/slice_001/cell_000/LSPS_dendrite_CC_testmap_MAX_002"
        )
        cell_vc = Path(
            "/Users/pbmanis/Desktop/Data/Glutamate_LSPS_DCN/2019.08.08_000/slice_001/cell_000/LSPS_dendrite_VC_testmap_MAX_000"
        )
        MT.setPars(
            {
                "invert": True,
    ...
    shiftkey = False
    maptable = Path(
        experiment["databasepath"], experiment["directory"], experiment["selectedMapsTable"]
    )
    ...
    def toggle_selector(event):
        ...
        # reset is 'r'
            ...
            MT.SI_minmax[1] = np.max((0.0, MT.SI_minmax[1] - 0.05))
            ...
            MT.SI_minmax[0] = np.max((0.0, MT.SI_minmax[0] - 0.05))
            if MT.SI_minmax[1] > MT.SI_minmax[0]:
        ...
        # grayscale image
            ...
            MT.cmax = np.min((65535, MT.cmax + 500.0))
            MT.imageax.set_clim(MT.cmin, MT.cmax)
            mpl.draw()
        elif event.key in ["-"]:
            MT.cmax = np.max((MT.cmin, MT.cmax - 500.0))
            MT.imageax.set_clim(MT.cmin, MT.cmax)
            mpl.draw()

        elif event.key in ["u"]:  # adjust min up or down
            MT.cmin = np.min((MT.cmax - 10.0, MT.cmin + 100.0))
            MT.imageax.set_clim(MT.cmin, MT.cmax)
            mpl.draw()

        elif event.key in ["d"]:
            MT.cmin = np.max((0, MT.cmin - 100.0))
            MT.imageax.set_clim(MT.cmin, MT.cmax)
            mpl.draw()
    ...
    def plot_a_cell(cellname, cellno, ax=None, axb=None, tbar=False):
        ...
        cell = Path(
            ...
            str(dc["cellID"].values[0]),
            str(dc["map"].values[0]),
        )
        imagename = dc["cellimage"].values[0]
        print("cell image: ", imagename)
        if (
            not pd.isnull(imagename)
            and imagename.startswith("image_")
            or imagename.startswith("../image_")
        ):
        ...
        MT.setPars(pars)
        ER = EventReader(
            Path(experiment["analyzeddatapath"], experiment["directory"]),
            dc["cellID"].values[0],
            dc["map"].values[0],
        )
        if ER.data is None:
        ...
        MT.setOutputFile(
            Path(
                ...
                experiment["directory"],
                f"{cellname:s}{int(cellno):d}_map.pdf",
            )
        )
        prots = {"ctl": cell}
        ...
        if len(sequence) > 1:  # all of a type
            ...
            for npage in range(npages):
                ...
                for axn in range(nperpage):
                    ...
                    print(
                        "cell, cell number: ", cs.iloc[icell]["cellname"], cs.iloc[icell]["cellno"]
                    )
                    plot_ok = plot_a_cell(
                        cs.iloc[icell]["cellname"],
                        cs.iloc[icell]["cellno"],
                        ax=axarr[axn],
                        tbar=tbar,
                        axb=axarr2[axn],
                    )
                    ...
                    axarr[axn].set_title(
                        f"{cname:s} {int(cs.iloc[icell]['cellno']):d}\n{cs.iloc[icell]['map']:s}",
                        fontsize=9,
                        horizontalalignment="center",
                    )
                    axarr2[axn].set_title(
                        f"{cname:s} {int(cs.iloc[icell]['cellno']):d}\n{cs.iloc[icell]['map']:s}",
                        fontsize=9,
                        horizontalalignment="center",
                    )
                ...
                P1.figure_handle.suptitle(
                    f"{cname:s} Celltype: {cellname[0]:s} Page: {npage:d}", fontsize=14
                )
                P2.figure_handle.suptitle(
                    f"{cname:s} Celltype: {cellname[0]:s} Page: {npage:d} distancemaps", fontsize=14
                )
                ymax = 0
                for ax2 in axarr2:
                    yl = ax2.get_ylim()
                ...
                mpl.show()
                P1.figure_handle.savefig(
                    Path(
                        "NF107Ai32_Het/Example Maps/",
                        f"{cellname[0]:s}{t:s}_{seq:s}_Page{npage:d}.pdf",
                    )
                )
                P2.figure_handle.savefig(
                    Path(
                        "NF107Ai32_Het/Example Maps/",
                        f"{cellname[0]:s}{t:s}_{seq:s}_Page{npage:d}_distmap.pdf",
                    )
                )
                # mpl.show()# mpl.close()
