...
from src.util import decorate_excel_sheets as DE
...
datasets, expt = get_configuration()

# actually need to select the data set
dataset = datasets[0]

location = expt[dataset]['region']
cell_types = expt[dataset]['celltypes']

DATA_DIRECTORY = expt[dataset]['rawdatapath']
...
def datenum(datestr: str):
    s = datestr.split(".")
    d = int(1e4 * int(s[0])) + int(1e2 * int(s[1])) + int(s[2])
    return d

def get_lowest_current_spike(row, SP):
    ...
    if len(dvdts) > 0:
        ...
        CP("y", f"I={currents[min_current]*1e12:6.1f} pA, dvdtRise={row.dvdt_rising:6.1f}, dvdtFall={row.dvdt_falling:6.1f}, APthr={row.AP_thr_V:6.1f} mV, HW={row.AP_HW*1e3:6.1f} usec")
    return row
...
def get_iv_protocol_from_pkl(
    ...
    fullpatha = Path(datapath, date, row.slice_slice, row.cell_cell, row.iv_name)
    ...
    with DR.acq4_reader.acq4_reader(fullpath, "MultiClamp1.ma") as AR:
        ...
        args.experiment = experiments[experiment]
        args.iv_flag = True
        args.map_flag = False
        args.autoout = True
        ...
        if not stable:
            CP("m", f"Cell is not stable (RMP SD = {row.rmp_sd:.4f}): {fullpath!s}\n")
            return row
        ...
        row = get_lowest_current_spike(row, IVA.SP)
...
def _get_iv_protocol(row, experiment, pdf_pages: Union[object, None] = None, axis=1):
    """Get the IV protocol from this dataframe row,
    ...
    row.date = row.date.replace("NF107ai32", "NF107Ai32")
    # if datenum(row.shortdate) != datenum("2019.05.06"):  # just for testing purposes...
    #     print("not targeted date ", row.date, row.ID, row.age, row.Group)
    #     return row
    # fulldate = Path(row.data_directory, row.date)
    # testdate = "2019.05.06_000"
    # if not str(fulldate).endswith(testdate): #  != Path("/Volumes/Pegasus_002/ManisLab_Data3/Kasten_Michael/NF107ai32_NIHL/BNE_3d/2019.05.06_000"):
    #     # print("not targeted date ", row.date, row.ID, row.age, row.Group)
    #     # Logger.info(f"not targeted date {row.date!s}, {row.ID!s}, {row.age!s}, {row.Group!s}")
    #     return row
    ...
    fullpatha = Path(
        DATA_DIRECTORY, dataset, row.date, row.slice_slice, row.cell_cell, row.protocol
    )
    if fullpatha.name.startswith("CCIV_1nA_max"):
        fullpath = fullpatha
    elif fullpatha.name.startswith("CCIV_1nA_Posonly"):
...
def find_protocols(
    ...
    df = df.apply(_data_complete_to_series, axis=1)
   ...
   # df = df.apply(_get_iv_protocol, experiment=exp, pdf_pages=pdf_pages, axis=1)
    df = DataFrameParallel(df, n_cores=N_WORKERS, pbar=True).apply(_get_iv_protocol, experiment=exp, pdf_pages=None, axis=1)
...
def process_spikes(experiment):

    exp = analyze_ivs.experiments[experiment]
