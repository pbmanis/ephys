...
class AnalyzeMap(object):
    ...
    def readProtocol(
        ...
        if self.stimtimes is not None:
            self.Pars.twin_base = [
                0.0,
                self.stimtimes["start"][0] - 0.001,
            ]  # remember times are in seconds
            self.Pars.twin_resp = []
            for j in range(len(self.stimtimes["start"])):
                self.Pars.twin_resp.append(
                    [
                        self.stimtimes["start"][j] + self.Pars.direct_window,
                        self.stimtimes["start"][j] + self.Pars.response_window,
                    ]
                )
        self.lbr_command = (
    ...
    def analyze_one_map(
        ...
        if self.Pars.LPF_flag or self.Pars.notch_flag or self.Pars.HPF_flag:
            if self.verbose:
                CP.cprint("m", f"LPF Filtering at {self.Pars.LPF:.2f} Hz")
    ...
    def plot_events(self, axh: object, results: dict, colorid: int = 0) -> None:
        ...
        if plotevents and len(eventtimes) > 0:
            ...
            bins = np.linspace(
                0.0, self.ar_tstart, int(self.ar_tstart * 1000.0 / 2.0) + 1
            )
            print(bins)
            print(y)
            axh.hist(
            ...
            axh.set_xlim(0.0, self.ar_tstart - 0.005)
                    ...
                    if events is not None and j in list(events.keys()):
                        ...
                        if len(events[j][i]["peaktimes"][0]) > 0:
                            ...
                            if (
                                ...
                                ax.plot(
                                    tb[tsi],
                                    ms * self.Pars.scale_factor + self.stepi * i,
                                    "o",
                                    color=ck,
                                    markersize=2,
                                ...
                                ax.plot(
                                    tb[tri],
                                    mr * self.Pars.scale_factor + self.stepi * i,
                                    "o",
                                    color=cr,
                                    markersize=2,
                                    markeredgecolor="None",
                                ...
                                ax.plot(
                                    tb[ts2i],
                                    ms2 * self.Pars.scale_factor + self.stepi * i,
                                    "o",
                                    color=cg,
                                    markersize=2,
                                    markeredgecolor="None",
    ...
    def plot_average_traces(
        ...
        ax.set_xlim(0.0, self.ar_tstart * 1e3 - 1.0)
        return
    ...
    def plot_photodiode(self, ax, tb, pddata, color="k"):
        ...
        ax.set_xlim(0.0, self.ar_tstart - 0.001)
    ...
    def display_position_maps(
        self, dataset_name: Union[Path, str], result: dict
    ) -> bool:
    ...
    def display_one_map(
        ...
        if self.photodiode:
            self.plot_photodiode(
                self.P.axdict["D"], self.AR.Photodiode_time_base[0], self.AR.Photodiode
            )
        # mpl.show()
