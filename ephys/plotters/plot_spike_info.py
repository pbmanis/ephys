...
excelsheet, analysis_cell_types, adddata = setup(experiment)
...
def read_data_files(excelsheet, adddata=None, analysis_cell_types=[]):
...
def combine_by_cell(
    df, celltypes=analysis_cell_types, plot_fits=False, valid_protocols=None
):
    """
...
def print_for_prism(row, celltype="tuberculoventral"):
...
def rescale_values_apply(row, measure, scale=1.0):
...
def summary_plot_spike_parameters_categorical(
    ...
    # print("Picker funcs: ", picker_funcs)
...
def pick_handler(event, picker_funcs):
...
def apply_scale(row, measure, scale):
    row[measure] = row[measure] * scale
    return row
...
def create_one_plot_categorical(
    ...
    dfp = data[data["cell_type"] == celltype]
    dfp = dfp.copy()
    dfp = dfp.apply(apply_scale, axis=1, measure=y, scale=yscale)
    if transform is not None:
...
def summary_plot_spike_parameters_continuous(df, logx=False, xlims=[0, max_age]):
    """Make a summary plot of spike parameters for selected cell types.
    ...
    for i, celltype in enumerate(showcells):
        ...
        for icol, measure in enumerate(measure_cols):
            ...
            picker_funcs[axp] = create_one_plot_continuous(
                data=dfp,
                x="age",
                y=measure,
                ax=axp,
...
def label_celltypes(P, analysis_cell_types):
    """label_celltypes 
...
def limit_to_max_rate_and_current(fi_data, imax=None, id="", limit=0.9):
    ...
    for i, celltype in enumerate(analysis_cell_types):
        ...
        for index in cdd.index:
            ...
            if FI_data.shape == (2, 0):  # no fi data from the excel table....
                continue
            FI_data = limit_to_max_rate_and_current(
        ...
        for i, g in enumerate(colors):
            if celltype == "pyramidal":
                ax.text(
                    ...
                    s=f"{experiment['group_legend_map'][g]:<24s}",
                    ha="left",
                    va="bottom",
                    fontsize=8,
                    color=colors[g],
...
def get_assembled_filename(experiment):
    if experiment["directory"].endswith("Het"):
        fn = Path(
            Path(
                experiment["analyzeddatapath"],
                experiment["directory"],
                "Het_combined_by_cell.pkl",
            )
        )
    elif experiment["directory"].endswith("NIHL"):
        fn = Path(
            Path(
                experiment["analyzeddatapath"],
                experiment["directory"],
                "NIHL_combined_by_cell.pkl",
            )
        )
    elif experiment["directory"].endswith("CBA_Age"):
        fn = Path(
            Path(
                experiment["analyzeddatapath"],
                experiment["directory"],
                "CBA_Age_combined_by_cell.pkl",
            )
        )
    return fn
...
def assemble_datasets(
    ...
    df = combine_by_cell(df, plot_fits=plot_fits)
...
def preload(fn):
    ...
    if experiment["excludeIVs"] is None:
        return df
    for fns in experiment["excludeIVs"]:
...
def do_stats(df, analysis_cell_types, divider="-" * 80):
    stat_text = " "
...
if __name__ == "__main__":
    ...
    fn = get_assembled_filename(experiment)
    ...
    divider = "=" * 80 + "\n"
    if args.stats:
        do_stats(df, analysis_cell_types, divider=divider)
    ...
    if args.plot in ["all", "spikes"]:
        P1, picker_funcs1 = summary_plot_spike_parameters_categorical(
            df,
            porder=experiment["plot_order"],
    ...
    if args.plot in ["all", "firing"]:
        ...
        P2, picker_funcs2 = summary_plot_spike_parameters_categorical(
            df,
            measure_cols=[
                "AdaptRatio",
                "FISlope",
    ...
    if args.plot == "rmtau":
        P3, picker_funcs3 = summary_plot_RmTau_categorical(
            df,
            porder=experiment["plot_order"],
            colors=experiment["plot_colors"],
            grouping="named",
    ...
    if args.plot in ["all", "fi"]:
        P4, picker_funcs4 = summary_plot_FI(
            df,
