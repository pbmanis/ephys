...
class MiniAnalyses:
    ...
    def summarize(self, data, order: int = 11, verbose: bool = False) -> None:
        ...
        avgwin = 5  # int(1.0/self.dt_seconds)  # 5 point moving average window for peak detection
        ...
        for itrial, dataset in enumerate(data):  # each trial/trace
            ...
            self.intervals.append(
                np.diff(self.timebase[self.onsets[itrial]])
            )  # event intervals
            ev_accept = []
    ...
    def fit_individual_events(self) -> None:
        ...
        self.ev_amp = (
            np.zeros(nevents) * np.nan
        )  # measured peak amplitude from the event itself
        self.ev_Qtotal = (
            np.zeros(nevents) * np.nan
