...
class AverageEvent:
    ...
    avgevent: Union[List, np.ndarray] = field(default_factory=def_empty_list)  # the event
...
class Mini_Event:
    ...
    event: np.ndarray = field(default_factory=def_empty_list)  # the event waveform
...
class Mini_Event_Summary:
    ...
    ok_onsets: Union[List, np.ndarray] = (
        field(  # onset times for detected events with artifact rejection
            default_factory=def_empty_list2D
        )
    )
    peakindices: Union[List, np.ndarray] = field(  # peak indices (not denoised) for detected events
        default_factory=def_empty_list
    )
    ...
    amplitudes: Union[List, np.ndarray] = field(default_factory=def_empty_list)  # event amplitudes
    Qtotal: Union[List, np.ndarray] = field(default_factory=def_empty_list)  # charge for each event
    crit: Union[List, np.ndarray] = field(  # peak times (not denoised) for detected events
        default_factory=def_empty_list
    )
    scale: Union[List, np.ndarray] = field(  # peak times (not denoised) for detected events
        default_factory=def_empty_list
    )
    ...
    allevents: Union[List, np.ndarray] = field(  # array of all events (2d, trace, event waveform)
        default_factory=def_empty_list
    )
    all_event_indices: Union[List, None] = field(  # list linking all events to parent trace
        default_factory=def_empty_list
    )
    # list linking isolated (clean) events to parent trace list
    isolated_event_trace_list: Union[List, None] = field(default_factory=def_empty_list)
    clean_event_onsets_list: Union[List, None] = field(default_factory=def_empty_list)
...
class Artifacts:
    ...
    starts: Union[List, None] = field(  # list of starting times for artifacts
        default_factory=def_empty_list
    )

    durations: Union[List, None] = field(  # list of artifact durations
        default_factory=def_empty_list
    )
...
def def_stim_artifacts():
    ...
    return {
        ...
        "durations": [0.0005, 0.0005, 0.0005, 0.0005, 0.0005],
    }
...
class AnalysisPars:
    ...
    stim_artifacts: dict = field(default_factory=def_stim_artifacts)
    ...
    response_window: float = (
        0.015  # window end for response (response is between direct and response), seconds
    )
    twin_base: list = field(default_factory=def_twin_base)  # time windows baseline measures
    twin_resp: list = field(
        default_factory=def_twin_response
    )  # time windows for repeated responses
    analysis_window: list = field(default_factory=def_analysis_window)  # window for data analysis
    # taus = [0.5, 2.0]
    template_tmax = 0.0025  # seconds
    template_pre_time: float = 0.0  # seconds
    risepower: float = 4.0
    taus: list = field(default_factory=def_taus)  # initial taus for fitting detected events
...
class AnalysisData:
    ...
    photodiode_timebase: Union[None, np.ndarray] = (
        None  # photodiode time base matching current trace
    )
