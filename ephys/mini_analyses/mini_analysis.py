...
class MiniAnalysis:
    def __init__(self, dataplan):
        ...
        ----------
        dataplan : object
            a dataplan object, with                    datasource: the name of the file holding the dict
    ...
    def analyze_one(
        ...
        for nprot, dprot in enumerate(mousedata["prots"]):
            ...
            if check:
                ...
                if result is False:
                    sys.stdout.write(
                        r"\n\033[0;31m******* Get data failed to find a file ",
                        fn,
                        dataname,
                        " *******\033[0;97m\n",
                    )
            ...
            if ext != ".mat":
                ...
                dts = acqr.sample_interval
                min_index = int(self.min_time / dts)
                if self.max_time > 0.0:
                    max_index = int(self.max_time / dts)
                else:
            ...
            if not datanameposted and not check:
                self.P.figure_handle.suptitle(
                    mouse + "  " + dataname + " : " + genotype,
                    fontsize=9,
                    weight="bold",
                )
                datanameposted = True
            data = data * 1e12  # convert to pA
            time_base = time_base * 1000.0
            maxt = np.max(time_base)
            tracelist, nused = self.analyze_block_traces(
                mode, data, time_base, maxt, dt, mousedata, exclude_traces, ntr
            )
        ...
        self.plot_individual_events(
            ...
            title=(
                "{0:s}  {1:s}  {2:s}".format(
                    dataname, self.cell_summary["mouse"], self.cell_summary["genotype"]
                )
            ),
            pdf=pdf,
        )
    ...
    def analyze_block_traces(
        self, mode, data, time_base, maxt, dt, mousedata, exclude_traces, ntr
    ):
        order = int(1.0 / dt)
        ...
        for i in tracelist:
            ...
            self.ax0.text(
                -1200, yp, "%03d %d" % (self.dprot, i), fontsize=8
            )  # label the trace
            ...
            if mode == "aj":
                ...
                cb.cbTemplateMatch(
                    data[i], threshold=float(mousedata["thr"]), order=order
                )
        ...
        for i, a in enumerate(tracelist):
            ...
            # linewidth=0.25, alpha=0.25, rasterized=self.rasterize)
            self.ax0.plot(
                time_base, data[i] + yp, "k-", linewidth=0.25, rasterized=self.rasterize
            )
            ...
            self.ax0.plot(
                onpk,
                method.Summary.smoothed_peaks[jtr[0]][jtr[1]] + yp,
                "ro",
                markersize=1.75,
