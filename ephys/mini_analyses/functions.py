...
def zeroCrossingEvents(data, minLength=3, minPeak=0.0, minSum=0.0, noiseThreshold=None, sign=0):
    ...
    minLength : int (default: 3)
        Minimum number of points for an "event" after a zero crossing
    
    minPeak : float (default: 0)
        If specified, events must have at least this amplitude to be accepted
    
    minSum : float (default: 0)
        If specified, events must have at least this sum over the length (e.g., charge)
    
    noiseThreshold : float (default: None)
        if specified, this is the threshold x amplitude std that an event 
        amplitude must exceed to be accepted.
    
    sign : int (default: None)
        if 0, accept all events
        if -1, accept only negative events
        if 1, accept only positive events
        Events accepted are according to their 'peak' value
    """
    ...
    if noiseThreshold  is not None:
        ...
        # print(np.fabs(events['sum']))
        # print(minSize)
        # print(sigma)
        mask = np.fabs(events['peak']) >= minSize
        #p.mark('mask')
        events = events[mask]
        #p.mark('select')

    if minPeak > 0:
        events = events[np.fabs(events['peak']) > minPeak]
    
    if minSum > 0:
        events = events[np.fabs(events['sum']) > minSum]
    
    if sign == -1: # negative events only
        events = events[events['peak'] < 0.]
    elif sign == 1: # positive events only
        events = events[events['peak'] > 0.]
    else:
        pass # all events
